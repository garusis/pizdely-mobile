'use strict';
/**
 * Created by garusis on 08/08/16.
 */
...
function loadConfig(env) {
    ...
    let conf = loadJsonFile(path.join(ENV.GULP_CONF_DIR, 'config.json'), ErrorConstant.WARNING);
    let confLocal = loadJsonFile(path.join(ENV.GULP_CONF_DIR, 'config.local.json'), ErrorConstant.WARNING);
    let confEnv = loadJsonFile(path.join(ENV.GULP_CONF_DIR, `config.${ENV[`${appName}_ENV`]}.json`), ErrorConstant.WARNING);
    let confEnvLocal = loadJsonFile(path.join(ENV.GULP_CONF_DIR, `config.${ENV[`${appName}_ENV`]}.local.json`), ErrorConstant.WARNING);

    ENV = _.defaultsDeep(confEnvLocal, confLocal, confEnv, conf, ENV);

    loadTasks(path.join(ENV.GULP_TASKS_DIR, 'tasks.js'), ErrorConstant.WARNING);
    loadTasks(path.join(ENV.GULP_TASKS_DIR, `tasks.${ENV[`${appName}_ENV`]}.js`), ErrorConstant.WARNING);
    loadTasks(path.join(ENV.GULP_TASKS_DIR, 'tasks.local.js'), ErrorConstant.WARNING);
    loadTasks(path.join(ENV.GULP_TASKS_DIR, `tasks.${ENV[`${appName}_ENV`]}.local.js`), ErrorConstant.WARNING);
